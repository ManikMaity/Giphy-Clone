...
function Categories() {
  ...
  const {data : categoryData, isFetched, isLoading, isError} = useQuery(["categories", subCategory], () => fetchCatagoryData(gf, categories, subCategory), {
    ...
    <div className='w-full flex flex-col sm:flex-row gap-5 min-h-[400px] my-4 md:my-6'>
        ...
        <div className='w-full sm:w-80'>
          ...
          {categoryData?.data && <div>
            ...
            <SubCategorySearch category= {categories} setSubCategory = {setSubCategory}/>
      ...
      <div className='w-full flex flex-col mb-4 md:mb-6'>
        ...
        <h2 className='font-extrabold text-2xl md:text-4xl'>{makeFirstCharUpperCase(categories)} GIFs</h2>
        {isFetched && <p className='font-light flex items-center gap-1'>{categoryData?.pagination?.total_count} Gifs
        <MdOutlineGifBox />
          </p>}

        </div>
        {(categories != subCategory && categoryData?.data) && <div className='flex justify-between'>
          <p className='my-3 text-xl font-extralight'>All {makeFirstCharUpperCase(subCategory)} Gifs</p>
          <CategoryGridBtn changeAlign={setAlign}/>
        </div>}


      <div className={ align == "row" ? `${`columns-1 w-3/4 my-5 mx-auto`}` :`${"columns-1 md:columns-2 xl:columns-3"} gap-2`}>
        {(isLoading || isError) && <div className="skeleton h-[800px] w-full"></div>}
        {isFetched && categoryData.data.slice(1).map(gifData => {
